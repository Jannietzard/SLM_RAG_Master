...
# ── TestEmbedQueryDocumentsConsistency (T-B) ─────────────────────────────────
...
class TestEmbedQueryDocumentsConsistency:
    ...
    def test_query_and_document_embeddings_have_same_dimension(self, embedder):
    ...
    def test_query_embedding_is_list_of_floats(self, embedder):
    ...
    def test_document_embedding_is_list_of_floats(self, embedder):
    ...
    def test_cosine_similarity_of_identical_text_near_one(self, embedder):
        ...
        """
        import math
        vec = [float(i + 1) / 768 for i in range(768)]  # non-trivial unit vector
...
# ── TestEmbeddingSemanticQuality (R-1, nightly + llm) ────────────────────────

class TestEmbeddingSemanticQuality:
    """Nightly semantic quality test using a live nomic-embed-text connection.

    Documents the known score-compression limitation (thesis §4.4): ALL text
    ...
    @pytest.mark.nightly
    @pytest.mark.llm
    def test_similar_texts_outscore_dissimilar_texts(self, tmp_db):
        """Semantically similar query must score higher against a related document
        than against a dissimilar one.

        R1-compliant: exactly 2 real inference calls.
...
# ── TestEmbedderErrorHandling (Action #5) ─────────────────────────────────────
...
class TestEmbedderErrorHandling:
    ...
    def test_connection_error_raises_runtime_not_raw(self, embedder):
    ...
    def test_http_500_raises_runtime_error(self, embedder):
