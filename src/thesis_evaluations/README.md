...
This directory contains every evaluation needed to defend the thesis:
...
| `agentic_ablation.py` | **Tier 1B** — the headline thesis table. Decomposes the pipeline (LLM-only → +Retrieval → +Planner → +Verifier → +SelfCorrect). | 5-row marginal-Δ table. |
...
## How the scripts map to the thesis title

| Title fragment | Supporting script(s) | Specific evidence |
|---|---|---|
...
### 4. Aggregate everything for the thesis
...
`.png` figures, and a `coverage_report.md` that confirms every thesis
...
| `pipeline_succeeded_llm_failed` | derived | The thesis's key cross-tab cell. |
...
Filter with `jq` for thesis examples:
...
These are valuable but **not required for a Master's thesis defence**. Each
