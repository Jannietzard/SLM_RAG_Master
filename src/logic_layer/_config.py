...
class ControllerConfig:
    ...
    For thesis-faithful values, callers must construct via
    ...
    Navigator Settings (pre-generative filtering, thesis section 3.3):
        ...
        enable_entity_overlap_pruning / enable_entity_mention_filter /
            enable_context_shrinkage: per-filter ablation toggles (thesis
    ...
    # Cross-encoder reranker (section 12.29)
    # Default-disabled in the dataclass (22 MB model download on first use);
    # settings.yaml currently has enable_reranker: true for the thesis runs.
    ...
    # Pre-generative filter toggles (thesis section 3.3 ablation rows).
    # These three filters are "original contributions" the thesis reports
    ...
    # split (the thesis methodology reports the dev/test partition).
    ...
    def __post_init__(self) -> None:
        if self.temperature != 0.0:
            warnings.warn(
                ...
                "thesis evaluation. Set llm.temperature in config/settings.yaml."
                % self.temperature,
                stacklevel=2,
            )
    ...
    def from_yaml(cls, config: Dict[str, Any]) -> "ControllerConfig":
        ...
        returned by this classmethod ARE the thesis-faithful configuration --
